...
function isCjmTestDeal(dealId) {
  // v95: при режиме "только новые сделки" никаких ускорений/обходов для Бобика нет.
  if (config.cjmNewDealsOnly) return false;
...
async function runServerAutopilotForDeal(deal, stageId) {
  ...
  try {
    ...
    if (String(deal.STAGE_ID || '') === String(expertAssignedStageId || '')) {
...
async function runAutopilotPollingCycle() {
  ...
  try {
    ...
    if (!config.cjmNewDealsOnly && config.cjmTestMode && config.cjmTestDealId) {
      try {
    ...
    if (config.cjmNewDealsOnly) {
      console.log(`[autopilot] v96 SAFETY: обрабатываю только сделки, созданные с ${config.cjmGoLiveAtIso}; все более старые сделки игнорируются.`);
    ...
    console.log(`[autopilot] Цикл: на целевых стадиях найдено ${allDeals.length}; новых после cutoff=${eligibleDeals.length}; старых пропущено=${skippedOld}.`);
