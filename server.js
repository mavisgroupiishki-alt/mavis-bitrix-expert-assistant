...
// v120: Bitrix24 opens local application placement URLs with POST.
// Serve the report page for both POST and GET before the local-app module is registered,
// so the placement never receives Express' `Cannot POST /doc-return-report`.
app.get('/doc-return-report', (_req, res) => {
  res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
  res.sendFile(path.join(__dirname, 'public', 'doc-return-report.html'));
});
app.post('/doc-return-report', (_req, res) => {
  res.setHeader('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate');
  res.sendFile(path.join(__dirname, 'public', 'doc-return-report.html'));
});
