...
const config = {
  ...
  // v95: CJM работает не по одной тестовой сделке, а по НОВЫМ сделкам Производства.
  // Все сделки, созданные ДО момента боевого запуска, навсегда исключаются из CJM.
  // Важно: cutoff фиксированный, чтобы рестарт Render не менял границу "старое/новое". Старый CJM_GO_LIVE_AT намеренно игнорируем; при необходимости используем CJM_GO_LIVE_AT_V96.
  cjmNewDealsOnly: String(process.env.CJM_NEW_DEALS_ONLY || 'true').toLowerCase() !== 'false',
  cjmGoLiveAtIso: process.env.CJM_GO_LIVE_AT_V96 || '2026-08-25T16:36:00+03:00',
  // Старый тестовый режим оставлен только для совместимости, но при CJM_NEW_DEALS_ONLY=true
  // он полностью игнорируется: Бобик и другие старые сделки не обрабатываются.
  ...
  cjmPilotOnly: String(process.env.CJM_PILOT_ONLY || 'false').toLowerCase() === 'true',
  cjmTestAllowNoCall: String(process.env.CJM_TEST_ALLOW_NO_CALL || 'false').toLowerCase() === 'true',
  // v98: единственная старая сделка, разрешённая для полного ускоренного CJM-теста — ООО Бобик.
  // Все остальные старые сделки по-прежнему жёстко исключены cutoff-ом.
  cjmBobikTestEnabled: String(process.env.CJM_BOBIK_TEST_ENABLED || 'true').toLowerCase() !== 'false',
  cjmTrainingTestMinutes: Math.max(1, Number(process.env.CJM_TRAINING_TEST_MINUTES || 2)),
  cjmRegistryTestMinutes: Math.max(1, Number(process.env.CJM_REGISTRY_TEST_MINUTES || 2)),
  attRegistryUrl: process.env.ATT_REGISTRY_URL || 'https://att.bsc.by/reestr',
  ...
  // v94: пока CJM в пилоте, входящие документы привязываем только к тестовой сделке.
  // Никаких массовых ответов по старым сделкам.
  clientDocsAllDeals: String(process.env.CLIENT_DOCS_ALL_DEALS || 'false').toLowerCase() === 'true',
  clientDocsTestDealId: process.env.CLIENT_DOCS_TEST_DEAL_ID || process.env.CJM_TEST_DEAL_ID || '38072',
...
app.post('/api/autopilot/reset/:dealId', async (req, res) => {
  ...
  try {
    const dealId = req.params.dealId;
    const gateDeal = await bitrixRestCall('crm.deal.get', { id: dealId }).catch(() => null);
    if (!gateDeal) return res.status(404).json({ ok: false, error: `Сделка ${dealId} не найдена` });
    if (!isCjmPilotAllowedDeal(gateDeal)) {
      return res.status(403).json({ ok: false, error: `Старая сделка: CJM не имеет права её сбрасывать или запускать. Cutoff ${config.cjmGoLiveAtIso}.` });
    }
    ...
    const toDelete = comments.filter((c) =>
      ...
      String(c.COMMENT || '').includes(AUTOPILOT_HUMAN_DONE) ||
...
app.post('/api/deals/siblings', async (req, res) => {
  ...
  try {
    ...
    const filtered = siblings.filter((s) => {
      ...
      if (!isCjmPilotAllowedDeal(s)) return false;
...
const EMAIL_POLL_INTERVAL_MS = Math.max(1, Number(process.env.EMAIL_POLL_INTERVAL_MINUTES || 1)) * 60 * 1000; // v92: по умолчанию 1 минута
let emailPollRunning = false;
async function runEmailPollSafely(trigger = 'interval') {
  if (emailPollRunning) {
    console.log(`[email] Предыдущий цикл ещё выполняется — ${trigger} пропускаю.`);
    return;
  }
  ...
  try {
    ...
    console.error(`[email] Ошибка цикла ${trigger}: ${e.message || e}`);
  } finally {
...
const CJM_GO_LIVE_AT_MS = (() => {
  const ms = Date.parse(String(config.cjmGoLiveAtIso || ''));
  return Number.isFinite(ms) ? ms : Date.parse('2026-08-24T12:37:00+03:00');
})();

function isCjmTestDeal(dealId) {
  // v98: Бобик — единственное намеренное исключение из cutoff для сквозного теста.
  // Это НЕ включает другие старые сделки и не меняет боевые сроки для новых сделок.
  return Boolean(config.cjmBobikTestEnabled) && String(dealId || '') === String(config.cjmTestDealId || '38072');
}

function cjmDealCreatedAtMs(deal) {
  if (!deal || typeof deal !== 'object') return NaN;
  const raw = deal.DATE_CREATE || deal.CREATED_TIME || deal.CREATED_AT || '';
  const ms = Date.parse(String(raw || ''));
  return Number.isFinite(ms) ? ms : NaN;
}

// v95: единый safety-gate для ВСЕГО CJM.
// По умолчанию разрешены только сделки, СОЗДАННЫЕ после CJM_GO_LIVE_AT.
// Старые сделки не становятся "новыми" из-за редактирования, нового письма или движения стадии.
function isCjmPilotAllowedDeal(dealOrId) {
  const id = typeof dealOrId === 'object' && dealOrId ? dealOrId.ID : dealOrId;
  if (isCjmTestDeal(id)) return true; // v98: только ООО Бобик (ID из CJM_TEST_DEAL_ID)
  if (config.cjmNewDealsOnly) {
    if (!dealOrId || typeof dealOrId !== 'object') return false;
    const createdMs = cjmDealCreatedAtMs(dealOrId);
    return Number.isFinite(createdMs) && createdMs >= CJM_GO_LIVE_AT_MS;
  }
  if (!config.cjmPilotOnly) return true;
  return String(id || '') === String(config.cjmTestDealId || '38072');
}
...
async function generateDocListDocx(deal) {
  try {
    ...
    const service = detectServiceFromDeal(deal);
    const docList = clientFacingDocList(getDocumentListForService(service));
...
async function checkUnassignedDeals() {
  ...
  try {
    ...
    for (const deal of deals) {
      if (!isCjmPilotAllowedDeal(deal)) continue;
...
const AUTOPILOT_MARKER = '[MAVIS_AUTOPILOT_DONE]'; // legacy: только чтение старых комментариев
const AUTOPILOT_HUMAN_DONE = 'ИИгорь: Ход работы отправлен клиенту.';
...
async function dealAlreadyProcessed(dealId) {
  ...
  // v91: успешный Ход работы блокирует дубли и для тестовой сделки.
  // Для повторного теста используем /api/autopilot/reset/:dealId.
  ...
  try {
    ...
    const done = comments.some((c) => { const t = String(c.COMMENT || ''); return t.includes(AUTOPILOT_MARKER) || t.includes(AUTOPILOT_HUMAN_DONE); });
...
async function addAutopilotCommentOnce(dealId, marker, body) {
  ...
  const humanSignature = String(body || '').trim().slice(0, 120);
  if (humanSignature && await timelineHasHumanSignature(dealId, humanSignature)) return false;
  await bitrixRestCall('crm.timeline.comment.add', {
    fields: { ENTITY_ID: dealId, ENTITY_TYPE: 'deal', COMMENT: String(body || '').trim() },
  });
  return true;
}
...
function activityPassesExpertGate(act, deal, opts = {}) {
  // v98: для единственной тестовой сделки ООО Бобик разрешаем использовать уже
  // существующий содержательный звонок из CRM, даже если он был до текущего
  // входа на стадию «Эксперт назначен» или его совершал предыдущий ответственный.
  // Для всех боевых сделок прежний gate остаётся без изменений.
  const allowHistoricalCall = Boolean(opts.allowHistoricalCall);
  const assignedId = allowHistoricalCall ? '' : String(opts.assignedById || (deal && deal.ASSIGNED_BY_ID) || '').trim();
  const minDateRaw = allowHistoricalCall ? '' : (opts.minDate || (deal && deal.MOVED_TIME) || '');
...
function getDocumentListForService(serviceText) {
  ...
  // v96: для СПК разделяем текущий сбор документов и будущий этап выезда.
  // Регистрация, устав и помещение НЕ уходят в первоначальный «Ход работы»/напоминания.
  ...
  const spkCollectionDocs = [
    'Копии дипломов и трудовой на заместителя директора/директора — 1 экз.',
    'Копии дипломов, аттестатов и трудовых на аттестованных сотрудников — 1 экз.',
    'Средства измерений: документы по фактически имеющимся СИ (накладные/договоры/поверки по необходимости) — 1 экз.',
  ...
  if (/(спк|стк)/.test(s) && /(аттест|атт)/.test(s)) return {
    title: 'Аттестация + СПК',
    docs: [...spkVisitDocs, ...spkCollectionDocs, ...commonAtt],
    collectionDocs: [...spkCollectionDocs, ...commonAtt],
    visitDocs: spkVisitDocs,
  };

  if (/(спк|стк)/.test(s)) return {
    title: 'СПК (Свидетельство технической компетентности)',
    docs: [...spkVisitDocs, ...spkCollectionDocs],
    collectionDocs: spkCollectionDocs,
    visitDocs: spkVisitDocs,
  };
  ...
  // v94: неизвестная/неподдерживаемая услуга НЕ получает чужой перечень.
  // Раньше fallback ошибочно возвращал Аттестацию СМР, из-за чего клиентам
  // по сертификации и другим услугам мог уходить неверный список документов.
  return null;
}
...
function clientFacingServiceName(service) {
  return String(service || '')
    .replace(/\bАттестация\s+СМР\b/gi, 'Аттестация')
    .replace(/\s{2,}/g, ' ')
    .trim();
}

function clientFacingDocList(docList) {
  const title = clientFacingServiceName(String((docList && docList.title) || ''));
  const sourceDocs = Array.isArray(docList && docList.collectionDocs)
    ? docList.collectionDocs
    : (Array.isArray(docList && docList.docs) ? docList.docs : []);
  const docs = sourceDocs
    .map((x) => String(x || '').trim())
    .filter(Boolean)
    .filter((x) => !/все копии заверяются|копия верна|подпись.*расшифровка.*печать/i.test(x));
  return { title, docs };
}

function buildClientDocumentSection(docList) {
  const clientList = clientFacingDocList(docList);
  const docs = clientList.docs;
  if (!docs.length) return '';
  const title = clientList.title;
...
function sanitizeSpkInitialClientMessage(text) {
  return String(text || '')
    .split(/\r?\n/)
    .filter((line) => {
      const t = String(line || '').toLowerCase();
      if (/свидетельств.*регистрац/.test(t)) return false;
      if (/\bустав\b/.test(t)) return false;
      if (/документ.*помещ|помещени.*юрадрес|помещени.*юридическ/.test(t)) return false;
      if (/книг.*уч[её]т.*провер|книг.*провер/.test(t)) return false;
      if (/копи[яи].*завер|копия верна|правил.*заверен/.test(t)) return false;
      return true;
    })
    .join('\n')
    .replace(/\n{3,}/g, '\n\n')
...
function finalizeClientWorkMessage(aiMessage, docList) {
  let base = clientFacingServiceName(removeForbiddenClientPhrases(cleanMarkdownForMessenger(stripClientGreeting(aiMessage))));
  const listTitle = String((docList && docList.title) || '');
  const isSpk = /спк|свидетельств.*техн|техн.*компетент/i.test(listTitle);
  if (isSpk) base = sanitizeSpkInitialClientMessage(base);
  // v96: для СПК — только персонализированные действия из разговора.
  // Универсальный перечень в конец первоначального «Хода работы» больше не приклеиваем.
  const docsSection = isSpk ? '' : buildClientDocumentSection(docList);
...
Что есть сейчас:
- специалисты: [ФИО / официальная должность / роль по СПК, только как прямо проговорено в звонке];
- средства измерений: [что именно клиент сообщил: есть / частично есть / нужно сверить];
- орган: [только если прямо определён];
- крайний срок: [дата].

От вас сейчас:
1. [только конкретное действие или документ, который реально нужен сейчас по разговору]
2. [следующее конкретное действие, если оно было согласовано]
3. [ещё одно действие только если оно действительно требуется сейчас]

От нас:
1. Проверяем специалистов и средства измерений
2. Сверяем фактические данные и документы по текущему этапу
3. Готовим документы для дальнейшего этапа
4. Подаем в согласованный орган и контролируем статус`;
  }
...
async function findSiblingDeals(deal, stageId) {
  ...
  try {
    ...
    return unique.filter((s) => String(s.ID) !== String(deal.ID) && isCjmPilotAllowedDeal(s));
  } catch (_) { return []; }
}
...
${String(reportComment).trim()}`
    : '';
  const taskDesc = `${petName}, я отправил клиенту персонализированный ход работы и записал всё в сделку.

Что сделал:
— проанализировал первый содержательный звонок;
— сформировал и отправил клиенту ход работы;
— зафиксировал необходимые на текущем этапе действия, документы и сроки;
...
async function extractSpecialistRolesFromTranscript(transcript, service, dealId = '') {
  const text = String(transcript || '').trim();
  if (!text || text.length < 40) return { specialists: [], ambiguities: [] };
  const system = [
    'Ты извлекаешь из расшифровки звонка структуру специалистов для MAVIS GROUP.',
    'КРИТИЧНО: официальная должность человека и роль, которую он закрывает в конкретной услуге, — разные поля.',
    'Ничего не додумывай. role_in_service заполняй только если эта роль прямо проговорена в звонке.',
    'Если сказано «Зайцева, главный инженер по общестрою, будет сметчиком», official_position = «главный инженер по общестроительным работам», role_in_service = «инженер-сметчик».',
    'Если сказано «Гопеев директор, его ставим главным инженером», official_position = «директор», role_in_service = «главный инженер».',
    'Если роль неясна — role_in_service=null и добавь причину в ambiguities.',
    'Верни только JSON.'
  ].join('\n');
  const user = `Услуга: ${service || 'не указана'}\n\nРасшифровка:\n${text.slice(0, 14000)}\n\nОтвет JSON:\n{"specialists":[{"name":"ФИО или фамилия как в звонке","official_position":"официальная должность или null","role_in_service":"функциональная роль по услуге или null","evidence":"короткий фрагмент звонка, подтверждающий именно эту роль","confidence":"high|medium|low"}],"ambiguities":["что не удалось однозначно понять"]}`;
  try {
    ...
    const specialists = Array.isArray(obj.specialists) ? obj.specialists.map((x) => ({
      name: String((x && x.name) || '').trim(),
      official_position: x && x.official_position ? String(x.official_position).trim() : null,
      role_in_service: x && x.role_in_service ? String(x.role_in_service).trim() : null,
      evidence: x && x.evidence ? String(x.evidence).trim().slice(0, 300) : '',
      confidence: ['high','medium','low'].includes(String(x && x.confidence || '').toLowerCase()) ? String(x.confidence).toLowerCase() : 'low',
    })).filter((x) => x.name) : [];
    const ambiguities = Array.isArray(obj.ambiguities) ? obj.ambiguities.map(String).filter(Boolean) : [];
    console.log(`[autopilot deal=${dealId || '?'} roles] ${JSON.stringify({ specialists, ambiguities })}`);
    return { specialists, ambiguities };
  } catch (e) {
    console.warn(`[autopilot deal=${dealId || '?'} roles] Не удалось отдельно разобрать роли специалистов: ${e.message || e}`);
    return { specialists: [], ambiguities: ['авторазбор ролей не выполнен'] };
  }
}

async function buildDealContext(deal, transcript) {
  const service = detectServiceFromDeal(deal);
  const rawDocList = getDocumentListForService(service);
  // v96: в контекст первоначального Хода работы по СПК не передаём документы будущего этапа выезда.
  // Так модель физически не видит регистрацию/устав/помещение при генерации стартового сообщения.
  const docList = rawDocList && Array.isArray(rawDocList.collectionDocs)
    ? { title: rawDocList.title, docs: rawDocList.collectionDocs, collectionDocs: rawDocList.collectionDocs }
    : rawDocList;
  const roleAnalysis = await extractSpecialistRolesFromTranscript(transcript, service, deal.ID);
  let commentsText = '';
  try {
    const comments = await bitrixRestList('crm.timeline.comment.list', {
  ...
  return {
    ...
    specialist_roles: roleAnalysis.specialists,
    specialist_role_ambiguities: roleAnalysis.ambiguities,
...
function buildNoCallTestWorkResult(deal, context, defaultDocDeadline) {
  const rawService = String((context && context.service) || detectServiceFromDeal(deal) || 'услуге').trim();
  const service = clientFacingServiceName(rawService);
...
async function hasOtherActiveProductionDealForCompany(companyId, currentDealId) {
  ...
  try {
    ...
    return deals.some((d) => String(d.ID) !== String(currentDealId || ''));
  } catch (e) {
    console.warn(`[cjm-duplicate-guard] Не удалось проверить активные сделки компании ${companyId}: ${e.message}`);
    // При ошибке проверки не блокируем новую сделку автоматически.
    return false;
  }
}
...
async function runServerAutopilotForDeal(deal, stageId) {
  ...
  try { deal = await loadFreshDeal(deal); } catch (_) {}
  if (!isCjmPilotAllowedDeal(deal)) {
    console.log(`${logPrefix} v96 SAFETY: старая сделка (создана до ${config.cjmGoLiveAtIso}) — CJM ничего не делает.`);
    return;
  }
  ...
  try {
    ...
    const serviceRaw = await resolveDealServiceName(deal) || detectServiceFromDeal(deal);
    ...
    if (!getDocumentListForService(serviceRaw)) {
      console.warn(`${logPrefix} v94 SAFETY: услуга "${serviceRaw || 'не определена'}" не поддерживается CJM Аттестация/СПК — клиенту ничего не отправляю.`);
      return;
    }

    // v97: если текущая сделка на стадии «Эксперт назначен», но у компании уже
    // есть другая активная сделка в Производстве — не отправляем Ход работы по текущей.
    // Это защищает клиента от повторных сообщений по нескольким услугам одной компании.
    const testDeal = isCjmTestDeal(dealId);
    const expertAssignedStageId = STAGE_IDS.expertAssigned || stageId || '';
    if (!testDeal && String(deal.STAGE_ID || '') === String(expertAssignedStageId || '')) {
      const hasOtherActive = await hasOtherActiveProductionDealForCompany(deal.COMPANY_ID, deal.ID);
      if (hasOtherActive) {
        console.log(`${logPrefix} v97 DUPLICATE-GUARD: у компании есть другая активная сделка в Производстве — Ход работы не отправляю.`);
        return;
      }
    }
    ...
    let callRecord = await findCallForDeal(dealId, {
      ...
      // v98: Бобик уже имеет содержательный звонок — новый звонок для теста не нужен.
      // Ищем среди всей истории звонков сделки, сохраняя требование длительности >=60 сек.
      allowHistoricalCall: testDeal,
    });
    ...
    // 3. Ищем сделки-компаньоны. В pilot-only режиме propagation запрещён:
    // Бобик не имеет права менять/комментировать соседние старые сделки компании.
    const siblings = (!config.cjmNewDealsOnly && config.cjmPilotOnly) ? [] : (stageId ? await findSiblingDeals(deal, stageId) : []);
    ...
    const systemPrompt = [
      ...
      'Если в context.specialist_roles есть role_in_service, это ПРИОРИТЕТНЫЙ факт из отдельного разбора разговора. Никогда не подменяй роль по услуге официальной должностью. Если они отличаются — укажи обе.',
      'Если роль специалиста не подтверждена напрямую или confidence=low — не придумывай её клиенту; вынеси сомнение только во внутренний comment.',
      'Не придумывай риски. Риск можно написать только если он прямо следует из звонка, документов или полей сделки. Если подтверждения нет — раздел «Риски» не нужен.',
      'Для СПК первоначальный Ход работы должен содержать только то, что реально нужно клиенту СЕЙЧАС по разговору. Не проси свидетельство о регистрации, устав и документ на помещение — это отдельный этап подготовки к выезду. Книгу учёта проверок не проси вообще.',
      'comment — ПОЛНЫЙ отчёт эксперту: что известно, специалисты/официальные должности/роли по услуге, что уже есть, чего не хватает, сроки и следующий шаг. Риски — только подтверждённые. Не ограничивайся 3-5 строками.',
      noCallTestMode
...
3. ДОКУМЕНТЫ И ДЕЙСТВИЯ:
   ...
   - По каждому специалисту, если он обсуждался, используй context.specialist_roles: отдельно официальная должность и отдельно роль по услуге.
   - Если в звонке прямо сказано, кем специалист будет по СПК, эта роль обязательна в тексте и не может быть заменена его официальной должностью.
   - НЕ пиши фразы «иные документы из обязательного перечня», «остальные необходимые документы», «прочие документы».
   - Для СПК НЕ перечисляй универсальный перечень и НЕ проси документы будущего этапа выезда (регистрация, устав, помещение). Проси только то, что реально требуется сейчас по разговору.
   - Книгу учёта проверок не проси.
   - Для других поддерживаемых услуг стандартный перечень может быть добавлен системой отдельно.
...
7. comment: полный рабочий отчёт эксперту. Обязательно перечисли:
   ...
   - дедлайны;
   - следующий шаг;
   - риск только если он ПРЯМО подтверждён разговором/документами/полями. Не добавляй гипотетические риски «на всякий случай».
...
Если срок не назван — укажи стандартный дедлайн +2 календарных дня, который дан выше.`;
    ...
    if (clientMessageWithEmail) {
      const phone = await getContactPhone(fullDeal);
      ...
      // v96: автоматическую отправку DOCX-перечня отключили.
      // Клиент получает одно персонализированное сообщение по фактической ситуации,
      // без второго универсального списка, который может дублировать уже присланные документы.
    ...
    for (const sibling of siblings) {
      try {
        ...
        const siblingComment = `${AUTOPILOT_HUMAN_DONE}\n\n${sendStatus}\n\n${dealComment}${crossNote}`;
        await bitrixRestCall('crm.timeline.comment.add', {
...
const ATT_STAGE4_MARKER = '[MAVIS_ATT_STAGE4_DONE]'; // legacy: только чтение старых комментариев
const ATT_STAGE4_HUMAN_DONE = 'ИИгорь: этап по личному кабинету Белстройцентра обработан.';
...
async function runAttStage4(deal, siblings = []) {
  if (!isCjmPilotAllowedDeal(deal)) return;
  const dealId = deal.ID;
  ...
  try {
    ...
    if (comments.some((c) => { const t = String(c.COMMENT || ''); return t.includes(ATT_STAGE4_MARKER) || t.includes(ATT_STAGE4_HUMAN_DONE); })) {
  ...
  if (lkCheck.found) {
    ...
    await bitrixRestCall('crm.timeline.comment.add', {
      fields: {
        ...
        COMMENT: `${ATT_STAGE4_HUMAN_DONE}\nИнформация про ЛК Белстройцентра уже есть в комментариях — продолжаем работу.`,
      },
    });
    return;
  }
  ...
  await bitrixRestCall('crm.timeline.comment.add', {
    fields: {
      ENTITY_ID: dealId, ENTITY_TYPE: 'deal',
      COMMENT: `${ATT_STAGE4_HUMAN_DONE}\nВопрос про ЛК Белстройцентра отправлен клиенту. Ждём ответ; ${petName} зафиксирует его в сделке.`,
    },
  });
...
const DOCS_REMINDER_MARKER = '[MAVIS_DOCS_REMINDER_DONE]'; // legacy: только чтение
const DOCS_REMINDER_HUMAN = 'ИИгорь: первый контроль документов выполнен.';
  ...
  try {
    ...
    if (analysis.action === 'send_reminder') {
      ...
      await bitrixRestCall('crm.timeline.comment.add', {
        fields: {
          ENTITY_ID: dealId, ENTITY_TYPE: 'deal',
          COMMENT: `${sent ? DOCS_REMINDER_HUMAN : 'ИИгорь: не удалось отправить первое напоминание по документам.'}\n${sent ? 'Напоминание клиенту отправлено.' : `Ошибка канала: ${sentResult.error || 'неизвестно'}`} ${analysis.situation ? `Ситуация: ${analysis.situation}.` : ''}`, 
        },
      });
      ...
      await bitrixRestCall('crm.timeline.comment.add', {
        fields: {
          ENTITY_ID: dealId,
          ENTITY_TYPE: 'deal',
          COMMENT: `${DOCS_REMINDER_HUMAN}\nПуш по документам не отправлялся: сначала нужно закрыть вопрос со специалистами. Контроль подбора идёт отдельным сценарием.`,
        },
      });
      ...
      await bitrixRestCall('crm.timeline.comment.add', {
        fields: {
          ENTITY_ID: dealId, ENTITY_TYPE: 'deal',
          COMMENT: `${DOCS_REMINDER_HUMAN}\nКлиентский пуш не отправлялся. ${analysis.comment}`, 
        },
      });
      ...
      await bitrixRestCall('crm.timeline.comment.add', {
        fields: {
          ENTITY_ID: dealId, ENTITY_TYPE: 'deal',
          COMMENT: `${DOCS_REMINDER_HUMAN}\nКлиентский пуш не требуется. Причина: ${analysis.reason || 'по контексту сделки'}.`,
        },
      }).catch(() => {});
    }
  ...
  for (const [dealId, trackInfo] of pendingDocsCheck.entries()) {
    ...
    try {
      ...
      const alreadyDone = comments.some((c) =>
        (String(c.COMMENT || '').includes(DOCS_REMINDER_MARKER) || String(c.COMMENT || '').includes(DOCS_REMINDER_HUMAN)) ||
...
function isCjmStopStage(deal) {
  const st = String(deal && deal.STAGE_ID || '');
  return st === String(STAGE_IDS.return) || st === String(STAGE_IDS.refund);
}
...
// ============================================================================
// v97: CJM БЛОК 5 — «Сбор информации»: 3 дня клиенту / 14 дней руководителю
...
async function clientDocsLatestStateWithTime(dealId) {
  try {
    ...
    for (const c of comments) {
      const text = String(c.COMMENT || '');
      let obj = null;
      if (text.includes(CLIENT_DOCS_STATE_MARKER)) obj = clientDocsParseMarkerJson(text, CLIENT_DOCS_STATE_MARKER);
      else if (text.includes(CLIENT_DOCS_HUMAN_PREFIX)) obj = clientDocsParseHumanStateComment(c);
      if (!obj) continue;
...
async function checkCollectionCjmBlock5() {
  ...
  try {
    ...
    const select = ['ID','TITLE','ASSIGNED_BY_ID','DATE_CREATE','MOVED_TIME','COMPANY_ID','CONTACT_ID',
    ...
    for (const deal of deals) {
      ...
      const leaderDue = testMin > 0 ? stageMinutes >= (testDeal ? 8 : testMin * 2) : stageDays >= Number(config.collectionLeaderDays || 14);
      ...
      if (leaderDue && !(await timelineHasHumanSignature(deal.ID, COLLECTION_14D_TEXT))) {
...
async function checkSelectionCjmBlock6() {
  ...
  try {
    ...
    const deals = await bitrixRestList('crm.deal.list', {
      ...
      select: [...new Set(['ID','TITLE','ASSIGNED_BY_ID','DATE_CREATE','MOVED_TIME','COMPANY_ID', ...selectionFieldCodesV85(fields)])],
      order: { MOVED_TIME: 'ASC' },
    }, 200);
    const now = new Date();
    for (const deal of deals) {
...
// ============================================================================
// v98: CJM БЛОКИ 7, 10, 11, 12 — безопасный мониторинг без клиентского спама
// Блок 8 «Передан оформителю» намеренно пропущен по бизнес-правилу.
// ============================================================================

const TRAINING_ONE_REMINDER_SIGNATURE = 'ИИгорь — единственное напоминание по обучению.';
const RETURN_STOP_SIGNATURE = 'ИИгорь: по сделке включён STOP-режим из-за возврата.';
const REGISTRY_FOUND_SIGNATURE = 'ИИгорь: компания найдена в реестре аттестатов соответствия.';
const registryLastCheckAt = new Map();

function parseRuDateToken(token) {
  const t = String(token || '').trim();
  let m = t.match(/^(\d{1,2})[.\/-](\d{1,2})[.\/-](\d{4})$/);
  if (m) {
    const d = new Date(Number(m[3]), Number(m[2]) - 1, Number(m[1]), 9, 0, 0, 0);
...
async function findTrainingInfoInTimeline(dealId) {
  try {
    const comments = await bitrixRestList('crm.timeline.comment.list', {
      filter: { ENTITY_ID: dealId, ENTITY_TYPE: 'deal' },
      select: ['ID','COMMENT','DATE_CREATE','CREATED'],
    ...
    for (const c of comments) {
      const text = String(c.COMMENT || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim();
      if (!/(обучен|экзамен|учеб|аттестац.*специалист)/i.test(text)) continue;
      const matches = text.match(/\b(?:\d{1,2}[.\/-]\d{1,2}[.\/-]\d{4}|\d{4}-\d{2}-\d{2})\b/g) || [];
      for (const token of matches) {
        const date = parseRuDateToken(token);
        if (date) return { date, text: text.slice(0, 800), token };
      }
      return { date: null, text: text.slice(0, 800), token: '' };
    }
  } catch (_) {}
  return null;
}

async function checkTrainingCjmBlock7() {
  if (!config.bitrixWebhookUrl || !config.autopilotEnabled) return;
  try {
    const deals = await bitrixRestList('crm.deal.list', {
      filter: { CATEGORY_ID: config.autopilotCategoryId || 28, STAGE_ID: STAGE_IDS.training },
      select: ['ID','TITLE','ASSIGNED_BY_ID','COMPANY_ID','DATE_CREATE','MOVED_TIME','STAGE_ID'],
      order: { MOVED_TIME: 'ASC' },
    }, 100);
    const now = new Date();
    for (const deal of deals) {
      if (!isCjmPilotAllowedDeal(deal) || isCjmStopStage(deal)) continue;
      if (await isDealAiDisabledAsync(deal)) continue;
      if (await timelineHasHumanSignature(deal.ID, TRAINING_ONE_REMINDER_SIGNATURE)) continue;

      const info = await findTrainingInfoInTimeline(deal.ID);
      const testDeal = isCjmTestDeal(deal.ID);
      const movedAt = new Date(deal.MOVED_TIME || deal.DATE_CREATE || 0);
      const elapsedMin = Number.isNaN(movedAt.getTime()) ? 0 : (now - movedAt) / 60000;
      let due = false;
      if (testDeal) due = elapsedMin >= Number(config.cjmTrainingTestMinutes || 2);
      else if (info && info.date) due = info.date.getTime() <= now.getTime() + 24 * 60 * 60 * 1000;
      if (!due) continue;

      const user = await getDistributionUserProfile(deal.ASSIGNED_BY_ID).catch(() => null);
      const expertName = user ? (user.NAME || '').trim() : 'Эксперт';
      const dateText = info && info.token ? `Дата из комментария: ${info.token}.` : 'В комментариях дата обучения/экзамена не распознана — проверь её вручную.';
      const dl = addWorkingDays(now, 1); dl.setHours(18, 0, 0, 0);
      await bitrixRestCall('tasks.task.add', { fields: {
        TITLE: `${expertName}, проверь обучение/экзамен по сделке`,
        DESCRIPTION: `По сделке «${deal.TITLE}» нужен контроль обучения/экзамена.\n\n${dateText}\n\nЭто единственное автоматическое напоминание по этому этапу.\n\nСделка: https://mavisgroup.bitrix24.by/crm/deal/details/${deal.ID}/`,
        RESPONSIBLE_ID: deal.ASSIGNED_BY_ID,
        DEADLINE: toMinskLocalIso(dl),
        UF_CRM_TASK: [`D_${deal.ID}`],
        PRIORITY: 1,
      ...
      await bitrixRestCall('crm.timeline.comment.add', { fields: {
        ENTITY_ID: deal.ID, ENTITY_TYPE: 'deal',
        COMMENT: `${TRAINING_ONE_REMINDER_SIGNATURE}\n${dateText} Эксперту поставлена одна задача на контроль; повторных автоматических напоминаний не будет.`,
      }}).catch(() => {});
      console.log(`[cjm-v98 training] deal=${deal.ID}: единственное напоминание эксперту создано.`);
    }
  } catch (e) { console.error('[cjm-v98 training]', e.message || e); }
}

function emailLooksLikeAuthorityRemark(act) {
  const text = String([act && act.SUBJECT, act && act.DESCRIPTION].filter(Boolean).join(' '))
    .replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').toLowerCase().replace(/ё/g, 'е');
  return /(замечан|устранен.*замеч|доработ|исправить|несоответств|отказ.*принят|возврат.*заяв|корректиров|необходимо.*предостав)/i.test(text);
}

async function checkAuthorityRemarksCjmBlock10() {
  if (!config.bitrixWebhookUrl || !config.autopilotEnabled) return;
  const stages = [STAGE_IDS.submitted, STAGE_IDS.filed, STAGE_IDS.checking, STAGE_IDS.remarks].filter(Boolean);
  try {
    for (const stageId of stages) {
      ...
      for (const deal of deals) {
        ...
        if (await isDealAiDisabledAsync(deal)) continue;
        const acts = await bitrixRestList('crm.activity.list', {
          filter: { OWNER_ID: deal.ID, OWNER_TYPE_ID: 2, TYPE_ID: 4 },
          order: { ID: 'DESC' },
          select: ['ID','SUBJECT','DESCRIPTION','DIRECTION','START_TIME','CREATED','LAST_UPDATED'],
        }, 30).catch(() => []);
        for (const act of acts) {
          if (String(act.DIRECTION || '') === '2') continue; // исходящее письмо
          if (!emailLooksLikeAuthorityRemark(act)) continue;
          const subject = String(act.SUBJECT || 'Без темы').replace(/\s+/g, ' ').trim().slice(0, 180);
          const when = String(act.START_TIME || act.CREATED || '').slice(0, 19);
          const signature = `ИИгорь: найдено письмо с замечанием — ${subject}`.slice(0, 220);
          if (await timelineHasHumanSignature(deal.ID, signature, 160)) continue;
          const excerpt = String(act.DESCRIPTION || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim().slice(0, 900);
          await bitrixRestCall('crm.timeline.comment.add', { fields: {
            ENTITY_ID: deal.ID, ENTITY_TYPE: 'deal',
            COMMENT: `${signature}\n${when ? `Дата письма: ${when}.\n` : ''}${excerpt ? `Содержание: ${excerpt}` : 'Открой письмо и проверь требования органа.'}`,
          }});
          console.log(`[cjm-v98 remarks] deal=${deal.ID}: замечание из email зафиксировано один раз.`);
          break; // одно новое замечание за цикл по сделке; следующее письмо возьмём позже
        }
      }
    }
  } catch (e) { console.error('[cjm-v98 remarks]', e.message || e); }
}

async function checkReturnStopCjmBlock11() {
  if (!config.bitrixWebhookUrl || !config.autopilotEnabled) return;
  try {
    for (const stageId of [STAGE_IDS.return, STAGE_IDS.refund].filter(Boolean)) {
      const deals = await bitrixRestList('crm.deal.list', {
        filter: { CATEGORY_ID: config.autopilotCategoryId || 28, STAGE_ID: stageId },
        select: ['ID','TITLE','ASSIGNED_BY_ID','COMPANY_ID','DATE_CREATE','MOVED_TIME','STAGE_ID'],
      }, 80);
      for (const deal of deals) {
        if (!isCjmPilotAllowedDeal(deal)) continue;
        if (await timelineHasHumanSignature(deal.ID, RETURN_STOP_SIGNATURE)) continue;
        const msg = `Возврат по сделке.\n\nКомпания/сделка: ${deal.TITLE}\nID сделки: ${deal.ID}\nИИгорь полностью остановил клиентские CJM-автоматизации по этой сделке.\n\nhttps://mavisgroup.bitrix24.by/crm/deal/details/${deal.ID}/`;
        await bitrixRestCall('im.message.add', { DIALOG_ID: TANYA_USER_ID, MESSAGE: msg }).catch(() => {});
        await bitrixRestCall('crm.timeline.comment.add', { fields: {
          ENTITY_ID: deal.ID, ENTITY_TYPE: 'deal',
          COMMENT: `${RETURN_STOP_SIGNATURE}\nАвтоматические сообщения клиенту по CJM отключены. Таня уведомлена один раз.`,
        }}).catch(() => {});
        console.log(`[cjm-v98 return] deal=${deal.ID}: STOP + Таня уведомлена.`);
      }
    }
  } catch (e) { console.error('[cjm-v98 return]', e.message || e); }
}

function normalizeCompanyTitleForRegistry(title) {
  return String(title || '').toLowerCase().replace(/ё/g, 'е')
    .replace(/[«»"']/g, '').replace(/\b(ооо|зао|оао|чуп|уп|ип|общество с ограниченной ответственностью)\b/gi, ' ')
    .replace(/[^a-zа-я0-9]+/gi, ' ').replace(/\s+/g, ' ').trim();
}

async function getCompanyUnpForRegistry(companyId) {
  if (!companyId) return '';
  try {
    const reqs = await bitrixRestList('crm.requisite.list', {
      filter: { ENTITY_TYPE_ID: 4, ENTITY_ID: companyId },
      select: ['ID','RQ_INN','NAME','ACTIVE'],
      order: { ID: 'DESC' },
    }, 20);
    for (const r of reqs) {
      const unp = String(r.RQ_INN || '').replace(/\D/g, '');
      if (unp.length >= 8) return unp;
    }
  } catch (_) {}
  return '';
}

async function fetchRegistryCandidate(url, init) {
  const ctrl = new AbortController();
  const timer = setTimeout(() => ctrl.abort(), 15000);
  try {
    const resp = await fetch(url, { redirect: 'follow', headers: { 'User-Agent': 'Mozilla/5.0 MAVIS-Igor/98', 'Accept': 'text/html,application/json,*/*' }, signal: ctrl.signal, ...(init || {}) });
    const text = await resp.text();
    return { ok: resp.ok, status: resp.status, text, url: resp.url || url };
  } finally { clearTimeout(timer); }
}

async function attRegistryContainsUnp(unp, companyTitle) {
  const base = String(config.attRegistryUrl || 'https://att.bsc.by/reestr').trim();
  const u = encodeURIComponent(unp);
  const name = encodeURIComponent(String(companyTitle || '').trim());
  const candidates = [
    `${base}?unp=${u}`,
    `${base}?search=${u}`,
    `${base}?q=${u}`,
    `${base}?query=${u}`,
    name ? `${base}?search=${name}` : '',
  ].filter(Boolean);
  let lastError = '';
  for (const url of candidates) {
    try {
      const r = await fetchRegistryCandidate(url);
      const plain = String(r.text || '').replace(/<script[\s\S]*?<\/script>/gi, ' ').replace(/<style[\s\S]*?<\/style>/gi, ' ').replace(/<[^>]+>/g, ' ').replace(/&nbsp;/gi, ' ').replace(/\s+/g, ' ');
      if (plain.includes(unp)) return { found: true, url: r.url, method: 'GET', status: r.status };
    } catch (e) { lastError = e.message || String(e); }
  }
  // Fallback для серверной формы реестра: пробуем типовые названия поля.
  for (const key of ['unp','search','q','query']) {
    try {
      const body = new URLSearchParams({ [key]: unp }).toString();
      const r = await fetchRegistryCandidate(base, { method: 'POST', headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' }, body });
      const plain = String(r.text || '').replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ');
      if (plain.includes(unp)) return { found: true, url: r.url, method: `POST:${key}`, status: r.status };
    } catch (e) { lastError = e.message || String(e); }
  }
  return { found: false, error: lastError };
}

async function checkAttRegistryCjmBlock12() {
  if (!config.bitrixWebhookUrl || !config.autopilotEnabled) return;
  try {
    const deals = await bitrixRestList('crm.deal.list', {
      filter: { CATEGORY_ID: config.autopilotCategoryId || 28, STAGE_ID: STAGE_IDS.won },
      select: ['ID','TITLE','COMPANY_ID','ASSIGNED_BY_ID','DATE_CREATE','MOVED_TIME','STAGE_ID'],
      order: { MOVED_TIME: 'DESC' },
    }, 100);
    const now = new Date();
    for (const deal of deals) {
      if (!isCjmPilotAllowedDeal(deal)) continue;
      if (await timelineHasHumanSignature(deal.ID, REGISTRY_FOUND_SIGNATURE)) continue;
      const testDeal = isCjmTestDeal(deal.ID);
      const last = registryLastCheckAt.get(String(deal.ID)) || 0;
      const minGapMs = testDeal ? Number(config.cjmRegistryTestMinutes || 2) * 60000 : 24 * 60 * 60 * 1000;
      if (last && now.getTime() - last < minGapMs) continue;
      registryLastCheckAt.set(String(deal.ID), now.getTime());

      const unp = await getCompanyUnpForRegistry(deal.COMPANY_ID);
      if (!unp) {
        console.warn(`[cjm-v98 registry] deal=${deal.ID}: УНП компании не найден в реквизитах — проверка реестра пропущена.`);
        continue;
      }
      let companyTitle = deal.TITLE || '';
      if (deal.COMPANY_ID) {
        try { const c = await bitrixRestCall('crm.company.get', { id: deal.COMPANY_ID }); companyTitle = (c && c.TITLE) || companyTitle; } catch (_) {}
      }
      const result = await attRegistryContainsUnp(unp, companyTitle);
      console.log(`[cjm-v98 registry] deal=${deal.ID}, UNP=${unp}: found=${result.found}${result.status ? `, HTTP=${result.status}` : ''}.`);
      if (!result.found) continue;
      await bitrixRestCall('crm.timeline.comment.add', { fields: {
        ENTITY_ID: deal.ID, ENTITY_TYPE: 'deal',
        COMMENT: `${REGISTRY_FOUND_SIGNATURE}\nУНП ${unp}. Проверка: ${config.attRegistryUrl}\nДальнейшие действия эксперт выполняет вручную; повторно ИИгорь это сообщение не создаст.`,
      }});
    }
  } catch (e) { console.error('[cjm-v98 registry]', e.message || e); }
}

async function runCjmExtendedMonitoringV98() {
  await checkTrainingCjmBlock7();
  // Блок 8 «Передан оформителю» — намеренно без автоматизации.
  await checkAuthorityRemarksCjmBlock10();
  await checkReturnStopCjmBlock11();
  await checkAttRegistryCjmBlock12();
}
...
async function runAutopilotPollingCycle() {
  ...
  try {
    ...
    if (config.cjmBobikTestEnabled && config.cjmTestDealId) {
      try {
        ...
        if (!testDeal || !testDeal.ID) {
          ...
          // v92: даже приоритетный CJM-тест НЕ имеет права повторно отправлять Ход работы,
          // если успешный человеческий маркер уже есть в таймлайне. Для повторного теста
          // используется только явный /api/autopilot/reset/:dealId.
          const testAlreadyDone = await dealAlreadyProcessed(testDeal.ID);
          if (testAlreadyDone) {
            console.log(`[autopilot-test] Сделка ${testDeal.ID}: Ход работы уже успешно отправлен — повтор не выполняю.`);
    ...
    if (config.cjmNewDealsOnly) {
      console.log(`[autopilot] v98 SAFETY: новые сделки с ${config.cjmGoLiveAtIso} + единственная тестовая сделка ${config.cjmTestDealId} (Бобик); остальные старые сделки игнорируются.`);
    } else if (config.cjmPilotOnly) {
      console.log(`[autopilot] legacy pilot-only ON — разрешена только сделка ${config.cjmTestDealId}.`);
    }

    // В v95 список стадий можно сканировать целиком: перед любым действием стоит жёсткий gate по DATE_CREATE.
    const allDeals = [];
    const seenIds = new Set();
    if (config.cjmNewDealsOnly || !config.cjmPilotOnly) for (const stageId of stageIds) {
      const filter = {
        CATEGORY_ID: config.autopilotCategoryId || 28,
        STAGE_ID: stageId,
      };
      ...
      const deals = await bitrixRestList('crm.deal.list', {
        filter,
        select: ['ID', 'TITLE', 'STAGE_ID', 'CATEGORY_ID', 'ASSIGNED_BY_ID', 'CONTACT_ID', 'COMPANY_ID', 'DATE_CREATE',
          'OPPORTUNITY', 'CURRENCY_ID', 'MOVED_TIME',
          process.env.SERVICE_FIELD_CODE || 'UF_CRM_1765113071',
    ...
    const eligibleDeals = allDeals.filter((deal) => isCjmPilotAllowedDeal(deal));
    const skippedOld = allDeals.length - eligibleDeals.length;
    console.log(`[autopilot] Цикл: на целевых стадиях найдено ${allDeals.length}; разрешено (новые + Бобик)=${eligibleDeals.length}; старых пропущено=${skippedOld}.`);
    ...
    await runCjmExtendedMonitoringV98();

    // v44: остальные старые сценарии стадий пока только по отдельному флагу.
    if (config.stageMonitoringEnabled && !config.cjmPilotOnly && !config.cjmNewDealsOnly) await runStageMonitoring();
...
const DOCS_WAIT_START_MARKER = '[MAVIS_DOCS_WAIT_START]'; // legacy: только чтение старых комментариев
const DOCS_WAIT_HUMAN_PREFIX = 'ИИгорь: контроль документов запущен.';
const CLIENT_DOCS_HUMAN_PREFIX = 'ИИгорь: получил документы через';
...
function formatMinskDateTimeForComment(date) {
  const parts = new Intl.DateTimeFormat('ru-RU', {
    timeZone: 'Europe/Minsk', day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit', hour12: false,
  }).formatToParts(new Date(date));
  const get = (t) => (parts.find((x) => x.type === t) || {}).value || '';
  return `${get('day')}.${get('month')}.${get('year')} ${get('hour')}:${get('minute')}`;
}

function parseHumanDocsWaitComment(comment, deal) {
  const text = String((comment && comment.COMMENT) || '');
  if (!text.includes(DOCS_WAIT_HUMAN_PREFIX)) return null;
  const m = text.match(/Срок контроля документов:\s*(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2})/i);
  const sentAt = new Date((comment && (comment.DATE_CREATE || comment.CREATED)) || Date.now());
  let dueAt = null;
  if (m) dueAt = new Date(`${m[3]}-${m[2]}-${m[1]}T${m[4]}:${m[5]}:00+03:00`);
  return {
    sentAt: sentAt.toISOString(),
    dueAt: dueAt && !Number.isNaN(dueAt.getTime()) ? dueAt.toISOString() : resolveDocumentsDueAt('', sentAt).toISOString(),
    companyName: String((deal && deal.TITLE) || ''),
    service: '',
    preferredChannel: '',
  };
}
...
async function persistDocsWaitStart(dealId, info) {
  const dueText = formatMinskDateTimeForComment(info.dueAt || Date.now());
  const channel = String(info.preferredChannel || '').trim();
  await bitrixRestCall('crm.timeline.comment.add', {
    fields: {
      ENTITY_ID: dealId,
      ENTITY_TYPE: 'deal',
      COMMENT: `${DOCS_WAIT_HUMAN_PREFIX}\nСрок контроля документов: ${dueText} (Минск).${channel ? `\nКанал связи: ${channel}.` : ''}`,
    },
  });
}
...
async function hasClientDocsReceivedAfter(dealId, afterDate) {
  ...
  try {
    ...
    for (const c of comments) {
      const text = String(c.COMMENT || '');
      if (!text.includes(CLIENT_DOCS_RECEIVED_MARKER) && !text.includes(CLIENT_DOCS_HUMAN_PREFIX)) continue;
      const payload = text.includes(CLIENT_DOCS_RECEIVED_MARKER) ? clientDocsParseMarkerJson(text, CLIENT_DOCS_RECEIVED_MARKER) : clientDocsParseHumanStateComment(c);
...
async function recoverPendingDocsChecks() {
  ...
  try {
    deals = await bitrixRestList('crm.deal.list', {
      filter: { CATEGORY_ID: config.autopilotCategoryId || 28, STAGE_ID: prepStageId },
      select: ['ID','TITLE','COMPANY_ID','CONTACT_ID','ASSIGNED_BY_ID','STAGE_ID','DATE_CREATE',
        config.serviceFieldCode || process.env.SERVICE_FIELD_CODE || 'UF_CRM_1765113071'],
      order: { ID: 'DESC' },
    }, 150);
  } catch (e) {
  ...
  for (const deal of deals) {
    ...
    try {
      ...
      for (const c of comments) {
        ...
        if (!wait && text.includes(DOCS_WAIT_HUMAN_PREFIX)) {
          wait = parseHumanDocsWaitComment(c, deal);
          if (wait) continue;
        }
      }
      ...
      for (const c of comments) {
        ...
        if (text.includes(CLIENT_DOCS_RECEIVED_MARKER) || text.includes(CLIENT_DOCS_HUMAN_PREFIX)) {
          const payload = text.includes(CLIENT_DOCS_RECEIVED_MARKER) ? clientDocsParseMarkerJson(text, CLIENT_DOCS_RECEIVED_MARKER) : clientDocsParseHumanStateComment(c);
        ...
        if (text.includes(DOCS_REMINDER_MARKER) || text.includes(DOCS_REMINDER_HUMAN)) reminderAfter = true;
        if (text.includes(CLIENT_DOCS_RECEIVED_MARKER) || text.includes(CLIENT_DOCS_HUMAN_PREFIX)) receivedAfter = true;
      }
...
function clientDocsRequiredDocs(docList) {
  const docs = Array.isArray(docList && docList.collectionDocs)
    ? docList.collectionDocs
    : (Array.isArray(docList && docList.docs) ? docList.docs : []);
...
function finalizeDocsReminderMessage(_baseText, docList) {
  ...
  return [
    actsMinskGreeting(),
    '',
    `Напоминаем по документам для услуги «${docList && docList.title ? clientFacingServiceName(docList.title) : 'вашей услуги'}». Пока не получили необходимый комплект.`,
    '',
...
async function sendClientTextByPreferredChannel(deal, text, subject = '') {
  // v94: после поиска сделки по email/телефону объект часто сокращённый и не содержит
  // enum-поля «Предпочитаемый канал связи». Перед отправкой всегда перечитываем полную сделку.
  let fullDeal = deal;
  try {
    const fresh = await bitrixRestCall('crm.deal.get', { id: deal.ID });
    fullDeal = fresh && (fresh.result || fresh) && (fresh.result || fresh).ID ? (fresh.result || fresh) : deal;
  } catch (_) {}
  if (!isCjmPilotAllowedDeal(fullDeal)) return { ok: false, skipped: true, error: 'старая сделка — CJM её не трогает' };

  const preferred = await detectPreferredChannelResolved(fullDeal);
  if (!preferred) return { ok: false, error: 'не заполнен/не распознан предпочитаемый канал связи' };

  if (preferred === 'email') {
    const email = await getContactEmail(fullDeal);
    if (!email) return { ok: false, channel: 'email', error: 'у клиента не найден email' };
    await sendEmailTextThroughBitrix(fullDeal, email, subject || `Документы: ${fullDeal.TITLE}`, text);
  ...
  await sendWazzupMessageInternal({ channelKey: preferred, text, phone, dealId: fullDeal.ID });
...
async function clientDocsFindDealsByComm(commType, commValue) {
  ...
  // v95: в боевом режиме работаем со всеми подходящими НОВЫМИ сделками.
  // Старые CLIENT_DOCS_ALL_DEALS=false / CLIENT_DOCS_TEST_DEAL_ID из пилота не сужают выбор до Бобика.
  const newDealsMode = Boolean(config.cjmNewDealsOnly);
  const testId = newDealsMode ? '' : String(config.clientDocsTestDealId || '').trim();
  if (!newDealsMode && !config.clientDocsAllDeals && !testId) {
  ...
  const companyIds = [...new Set(actsArrayIds(dup && (dup.COMPANY || dup.company)).map(String))];
  const serviceField = config.serviceFieldCode || process.env.SERVICE_FIELD_CODE || 'UF_CRM_1765113071';
  const dealSelect = ['ID','TITLE','CATEGORY_ID','STAGE_ID','STAGE_SEMANTIC_ID','CLOSED','ASSIGNED_BY_ID','CONTACT_ID','COMPANY_ID','DATE_CREATE','MOVED_TIME','DATE_MODIFY',serviceField];
  ...
  for (const d of all) {
    ...
    if (!isCjmPilotAllowedDeal(d)) continue;
    if (isCjmStopStage(d)) continue; // v98: на возврате клиентские автоответы полностью выключены
    if (!newDealsMode && !config.clientDocsAllDeals && testId && String(d.ID) !== testId) continue;
  ...
  if (newDealsMode || config.clientDocsAllDeals) {
...
function clientDocsParseHumanStateComment(comment) {
  const text = String((comment && comment.COMMENT) || '');
  if (!text.includes(CLIENT_DOCS_HUMAN_PREFIX)) return null;
  const lines = text.split(/\r?\n/);
  const docs = [];
  const missing = [];
  let section = '';
  let complete = /Статус комплекта:\s*полный/i.test(text);
  let source = '';
  const sourceMatch = text.match(/ИИгорь: получил документы через\s+([^\n.]+)/i);
  if (sourceMatch) source = sourceMatch[1].trim();
  for (const raw of lines) {
    const line = raw.trim();
    if (/^Распознано:/i.test(line)) { section = 'docs'; continue; }
    if (/^Не хватает:/i.test(line)) { section = 'missing'; continue; }
    if (/^(По специалистам:|Нужно проверить вручную:|Статус комплекта:)/i.test(line)) { section = ''; continue; }
    if (!/^—\s*/.test(line)) continue;
    const value = line.replace(/^—\s*/, '').trim();
    if (section === 'missing') { missing.push(value); continue; }
    if (section === 'docs') {
      const parts = value.split(/\s*\|\s*/);
      const obj = { fileName: '', documentType: 'документ', person: null, instrument: null, confidence: 'medium' };
      for (const part of parts) {
        const m = part.match(/^([^:]+):\s*(.*)$/);
        if (!m) continue;
        const k = m[1].trim().toLowerCase(); const v = m[2].trim();
        if (k === 'файл') obj.fileName = v;
        else if (k === 'тип') obj.documentType = v;
        else if (k === 'фио') obj.person = v || null;
        else if (k === 'си') obj.instrument = v || null;
        else if (k === 'уверенность') obj.confidence = v || 'medium';
      }
      if (obj.fileName || obj.documentType) docs.push(obj);
    }
  }
  return {
    at: new Date((comment && (comment.DATE_CREATE || comment.CREATED)) || Date.now()).toISOString(),
    source,
    docs,
    complete,
    missing,
...
async function clientDocsLoadState(dealId) {
  try {
    ...
    for (const c of comments) {
      const text = String(c.COMMENT || '');
      if (text.includes(CLIENT_DOCS_STATE_MARKER)) {
        const obj = clientDocsParseMarkerJson(c.COMMENT, CLIENT_DOCS_STATE_MARKER);
        if (obj) return obj;
      }
      if (text.includes(CLIENT_DOCS_HUMAN_PREFIX)) {
        const obj = clientDocsParseHumanStateComment(c);
        if (obj) return obj;
      }
    }
  } catch (_) {}
...
- Для СПК на текущем этапе «Сбор информации» оценивай только переданный выше текущий перечень. Не требуй регистрацию, устав и помещение — они относятся к будущему этапу выезда.
- Книгу учёта проверок не требуй.
...
function clientDocsBuildClientUpdate(results) {
  ...
  for (const r of results) {
    lines.push('', `По услуге «${clientFacingServiceName(r.service)}»:`);
...
function clientDocsIsRelevantForCompleteness(doc) {
  if (!doc) return false;
  const type = clientDocsNormalizeText(doc.documentType || '');
  const confidence = String(doc.confidence || 'low').toLowerCase();
  if (!type || /^(другое|неизвестно|unknown|документ)$/.test(type)) return false;
  if (doc.readable === false) return false;
...
async function clientDocsProcessIncomingAttachments({ source, commType, commValue, messageText = '', attachments = [] }) {
  ...
  const deals = match.deals.filter((d) => isCjmPilotAllowedDeal(d));
  if (!deals.length) return { ok: true, processed: 0, reason: 'outside-cjm-pilot' };
  ...
  const relevantDocs = analyzedDocs.filter(clientDocsIsRelevantForCompleteness);
  const irrelevantDocs = analyzedDocs.filter((d) => !clientDocsIsRelevantForCompleteness(d));
  if (irrelevantDocs.length) {
    console.log(`[client-docs] ${source}: ${irrelevantDocs.length} файл(ов) сохранены, но НЕ засчитываются в комплект (другое/нечитаемо/low confidence).`);
  }
  ...
  for (const deal of deals) {
    try {
      ...
      const mergedDocs = clientDocsMergeDocs(oldState.docs, relevantDocs);
      const completeness = relevantDocs.length
        ? await clientDocsCheckCompleteness(deal, service, mergedDocs)
        : (oldState && Array.isArray(oldState.docs) && oldState.docs.length
          ? await clientDocsCheckCompleteness(deal, service, oldState.docs)
          : null);
      ...
      const readableDocs = analyzedDocs.map((d) => {
        const parts = [`Файл: ${d.fileName}`, `Тип: ${d.documentType || 'документ'}`];
        if (d.person) parts.push(`ФИО: ${d.person}`);
        if (d.instrument) parts.push(`СИ: ${d.instrument}`);
        parts.push(`Уверенность: ${d.confidence || 'low'}`);
        return `— ${parts.join(' | ')}`;
      }).join('\n') || '— файл сохранён, тип требует проверки';
      const humanStateComment = relevantDocs.length ? [
        `${CLIENT_DOCS_HUMAN_PREFIX} ${source} и сохранил их в папку компании.`,
        `Статус комплекта: ${completeness && completeness.complete ? 'полный' : 'неполный'}.`,
        '',
        'Распознано:',
        readableDocs,
        '',
        ...
        `ИИгорь: получил файл через ${source} и сохранил его в папку компании.`,
        '',
        'Файл не засчитан в комплект документов по услуге и требует проверки экспертом:',
        readableDocs,
        '',
        'Срок ожидания документов не изменён. Клиенту автоматический ответ по этому файлу не отправлялся.',
      ].join('\n');
      ...
      await bitrixRestCall('crm.timeline.comment.add', {
        fields: {
          ENTITY_ID: deal.ID,
          ENTITY_TYPE: 'deal',
          COMMENT: humanStateComment,
        },
      });

      if (relevantDocs.length) {
        pendingDocsCheck.delete(String(deal.ID)); // релевантный документ меняет состояние комплекта
  ...
  if (resultPerDeal.length) {
    ...
    await bitrixRestCall('crm.timeline.comment.add', {
      fields: {
        ...
        ENTITY_TYPE: 'deal',
        COMMENT: `ИИгорь: ${send.ok ? `отправил клиенту обновлённый статус по документам через ${send.channel}` : `не смог отправить клиенту обновлённый статус по документам: ${send.error || 'ошибка'}`}.`,
      },
    }).catch(() => {});
  }
...
app.listen(PORT, () => {
  ...
  if (process.env.MAIL_IMAP_USER && process.env.MAIL_IMAP_PASSWORD && config.bitrixWebhookUrl) {
    console.log(`[email] Обработка почты включена (интервал ${EMAIL_POLL_INTERVAL_MS / 60000} мин; первый цикл через 15 сек).`);
    setTimeout(() => {
      runEmailPollSafely('startup');
      setInterval(() => runEmailPollSafely('interval'), EMAIL_POLL_INTERVAL_MS);
    }, 15 * 1000);
  } else {
