...
function productAiGuidance(productRaw, scenarioRaw = '') {
  ...
  const scenarioHints = {
    ...
    executor_attestation_call: [
      ...
      '',
      'document_message — ОТДЕЛЬНОЕ второе сообщение. Если специалисты есть — полный перечень из context.document_list.docs построчно. Если специалистов нет и их ищут — список требований к нужным специалистам (должность, образование, стаж, аттестат). Формат: "Перечень документов для [услуга]:" или "Требования к специалистам:" затем список через тире.',
      '',
